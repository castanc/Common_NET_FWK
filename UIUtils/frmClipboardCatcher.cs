...
namespace UIUtils
{
    ...
    public delegate void delegateClipboardCaptureImage(string fName);
    public delegate void delegateFormClosed();
    public partial class frmClipboardCatcher : Form
    {
        
        public static string OutputFolder { set; get; }

        private static string printScreenFolder = "";
        public static string PrintScreenFolder
        {
            set
            {
                if ( !string.IsNullOrEmpty(value ))
                {
                    printScreenFolder = value;
        ...
        {
            InitializeComponent();
            Message = @"Clipboard text and Print Screen are being monitored while this window is open.  To disable it, close this window.";
            Prefix = "PrintScreen_";
            Count = 0;
            PrintScreenFolder = @"c:\temp";
        }
        #endregion
        ...
        public event delegateClipboardCaptureImage OnClipboardCaptureImage = null;
        ...
        private bool ClipboardSearch(IDataObject iData)
        {
            ...
            {
                string fileName =  $@"{OutputFolder}//{Prefix}{Count.ToString().PadLeft(4, '0')}.jpg";
                while ( File.Exists(fileName))
                {
                    Count++;
                    fileName = $@"{OutputFolder}//{Prefix}{Count.ToString().PadLeft(4, '0')}.jpg";
                }
                ...
                OnClipboardCaptureImage?.Invoke(fileName);
                //return true;
                return false;
            }
